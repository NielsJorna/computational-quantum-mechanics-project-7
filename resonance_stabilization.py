...
def get_dipole(L, N, psi0_real, r_fixed):
    dr = r_fixed[1] - r_fixed[0]
    n_vec = np.arange(1, N + 1).reshape(-1, 1)
    
    # Current-L basis on the same fixed grid
    phi_L = np.sqrt(2/L) * np.sin(n_vec * np.pi * r_fixed / L)
    phi_L[:, r_fixed > L] = 0.0   # zero outside current box
    
    M = phi_L @ (psi0_real * r_fixed) * dr   # (N,)
    return M
...
L_ref = L_min
T_ref = np.diag(T_num / L_ref**2)
V_ref = V_matrix_sphere(L_ref, N, A)
energies_ref, evecs_ref = sp.linalg.eigh(T_ref + V_ref, subset_by_index=[0, 49])
ground_state_evec_fixed = evecs_ref[:, 0]

r_fixed = np.linspace(0, L_ref, 4000)
n_vec = np.arange(1, N + 1).reshape(-1, 1)
phi_ref = np.sqrt(2/L_ref) * np.sin(n_vec * np.pi * r_fixed / L_ref)
psi0_real = ground_state_evec_fixed @ phi_ref   # shape (4000,)
...
for L in tqdm(L_arr):
    ...
    energies, evecs = sp.linalg.eigh(H, subset_by_index=[0, 49])

    # Recompute ground state wavefunction on a grid scaled to current L
    r_local = np.linspace(0, L, 4000)
    n_vec = np.arange(1, N + 1).reshape(-1, 1)
    phi_local = np.sqrt(2/L) * np.sin(n_vec * np.pi * r_local / L)
    psi0_local = evecs[:, 0] @ phi_local  # ground state in real space

    # Use get_dipole with current-L grid and current ground state
    M_vector = get_dipole(L, N, psi0_local, r_local)
...
# Left plot — with ylim
ax1.plot(L_arr, eigs_arr, 'k-', lw=0.5, alpha=1)
ax1.axhline(Er_approx, color='r', lw=1, linestyle='--', 
            label=f'Literature $E_r$ = {Er_approx}', alpha=0.5)
ax1.set_xlabel("Box Size $L$")
ax1.set_ylabel("Energy $E$")
ax1.set_title("Stabilization Graph (zoomed)")
ax1.set_ylim(2, 5)
ax1.legend()

# Right plot — without ylim
ax2.plot(L_arr, eigs_arr, 'k-', lw=0.5, alpha=0.6)
ax2.axhline(Er_approx, color='r', lw=1, linestyle='--', 
            label=f'Literature $E_r$ = {Er_approx}', alpha=0.5)
ax2.set_xlabel("Box Size $L$")
ax2.set_ylabel("Energy $E$")
ax2.set_title("Stabilization Graph (full range)")
ax2.legend()
...
initials = [0.001, 0.01, 3.45, 0.025]
popt, pcov = curve_fit(
...
fano_mask = (e_centers > 3.1) & (e_centers < 3.8)

initials_fano = [0.001, 0.1, 3.45, 10, 0.03, 1]

popt_fano, pcov_fano = curve_fit(
    fano_fit, e_centers[fano_mask], rho_fano[fano_mask], p0=initials_fano
)
